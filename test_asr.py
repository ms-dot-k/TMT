...
def parse_args():
    ...
    parser.add_argument("--save_name", default='TMT')
...
def test(model):
    with torch.no_grad():
        ...
        description = 'Test ASR & TTS'
        if args.local_rank == 0:
            print(description)
        for i, batch in enumerate(dataloader):
            if args.local_rank == 0 and i % 10 == 0:
                print("******** Test : %d / %d ********" % ((i + 1) * batch_size, samples))
            _, sp_unit, txt_unit, sp_unit_len, txt_unit_len, f_names = batch
            ...
            if args.sp_txt:
                ...
                if args.local_rank == 0:
                    for st_result, gt_txt, f_name in zip(st_results, st_gt_txt, f_names):
                        save_name = os.path.join(args.temp_dir, 'st_transcription', f_name + '.txt')
                        save_gt_name = os.path.join(args.temp_dir, 'st_gt_transcription', f_name + '.txt')
                        if not os.path.exists(os.path.dirname(save_name)):
                            os.makedirs(os.path.dirname(save_name))
                        with open(save_name, 'w') as txt:
                            txt.write(st_result)
                        if not os.path.exists(os.path.dirname(save_gt_name)):
                            os.makedirs(os.path.dirname(save_gt_name))
                        with open(save_gt_name, 'w') as txt:
                            txt.write(gt_txt)

                if args.local_rank == 0:
                    if (i % 10) == 0:
                        print("*" * 10, "Speech -> Text", "*" * 10)
                        for j in range(sp_unit.size(0)):
            ...
            if args.txt_sp:
                ...
                if args.local_rank == 0:
                    for pred_sp, gt_txt, f_name in zip(pred_sp_unit, st_gt_txt, f_names):
                        save_name = os.path.join(args.temp_dir, 'ts_unit', f_name + '.unit')
                        save_gt_name = os.path.join(args.temp_dir, 'ts_gt_transcription', f_name + '.txt')
                        if not os.path.exists(os.path.dirname(save_name)):
                            os.makedirs(os.path.dirname(save_name))
        ...
        if args.txt_sp:
            ...
            text_generation(args, task='ts')
            ts_wer = WER_generation(args, task='ts')
            print("T => S WER: ", ts_wer)
            with open(os.path.join(args.temp_dir, 'ts_wer.txt'), 'w') as txt:
                txt.write(f"{ts_wer}")
        
        if st_wer is not None:
            print("S => T WER: ", st_wer)
            with open(os.path.join(args.temp_dir, 'st_wer.txt'), 'w') as txt:
                txt.write(f"{st_wer}")
...
def WER_generation(args, task='ts'):
    num_error = 0
    num_gts = 0
    txt_paths = glob.glob(f"{os.path.join(args.temp_dir, f'{task}_transcription')}/*.txt")
    for txt_path in txt_paths:
        with open(txt_path, 'r') as txt:
            pred = txt.readline().strip().lower()
        with open(os.path.join(args.temp_dir, f'{task}_gt_transcription', os.path.basename(txt_path)), 'r') as txt:
            gt = txt.readline().strip().lower()
        num_error += editdistance.eval(pred.split(), gt.split())
        num_gts += len(gt.split())
    return num_error / num_gts
